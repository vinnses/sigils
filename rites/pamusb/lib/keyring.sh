...
keyring_unlock_runtime() {
    ...
    local uid
    uid=$(id -u)
    local log_file="${XDG_RUNTIME_DIR:-/tmp}/pamusb-keyring.log"
    local hostname_target
    hostname_target=$(hostname)

    exec >> "$log_file" 2>&1
    printf '%s [INFO] pamusb keyring unlock started\n' "$(date '+%Y-%m-%d %H:%M:%S')"
...
keyring_install_unlock_script() {
    ...
    local script="${target_dir}/pamusb-unlock-keyring.sh"
    ...
    cat > "$script" <<EOF
#!/bin/bash
# pamusb keyring unlock wrapper — generated by pamusb setup
# DO NOT EDIT — regenerate with: pamusb setup-keyring
...
keyring_status() {
    ...
    # Sudoers
    if [[ -f /etc/sudoers.d/99-pamusb ]]; then
        print_ok "Sudoers drop-in: /etc/sudoers.d/99-pamusb"
    else
        print_fail "Sudoers drop-in: not installed"
    fi

    # Autostart
    local autostart="${HOME}/.config/autostart/pamusb-keyring.desktop"
    if [[ -f "$autostart" ]]; then
    ...
    # Unlock script
    local unlock_script="${HOME}/.local/bin/pamusb-unlock-keyring.sh"
    ...
    # udev suppress rule
    if [[ -f /etc/udev/rules.d/91-pamusb-suppress.rules ]]; then
        print_ok "udev suppress rule: /etc/udev/rules.d/91-pamusb-suppress.rules"
    else
...
keyring_uninstall() {
    ...
    # udev suppress rule
    if [[ -f /etc/udev/rules.d/91-pamusb-suppress.rules ]]; then
        rm -f /etc/udev/rules.d/91-pamusb-suppress.rules
